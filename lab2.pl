...
sub comments{
	...
	foreach my $title (@tracks) { 
		...
		##########################
		$title =~ s/\(.*//; # Parnethesis
		$title =~ s/\[.*//; # hard brackets
		$title =~ s/\{.*//; # Curly brackets
		$title =~ s/\\.*//; # \
		$title =~ s/\/.*//; # /
		$title =~ s/\_.*//; #_
		$title =~ s/\-.*//; # - 
...
sub clean{
	...
	foreach my $title (@tracks) { 
		...
		##########################
		#regex scan for ascii characters; if not empty skip
		next if($title=~s/[^0-9a-zA-Z'\s]//g);
		#next if($title=~s/[^[:ascii:]]//g);
...
sub stopwords{
	...
	foreach my $title (@tracks) { 
		#$title =~ s/[-!.;:&*_@%#|]//;
			...
			$title =~ s/(\ba)+\s//g;#a, 
			$title =~ s/(\ban)+\s//g;#an, 
			$title =~ s/(\band)+\s//g;#and, 
			$title =~ s/(\bby)+\s//g;#by, 
			$title =~ s/(\bfor)+\s//g;#for, 
			$title =~ s/(\bfrom)+\s//g;#from, 
			$title =~ s/(\bin)+\s//g;#in, 
			$title =~ s/(\bof)+\s//g;#of, 
			$title =~ s/(\bon)+\s//g;#on, 
			$title =~ s/(\bor)+\s//g;#or, 
			$title =~ s/(\bout)+\s//g;#out, 
			$title =~ s/(\bthe)+\s//g;#the, 
			$title =~ s/(\bto)+\s//g;#to, 
			$title =~ s/(\bwith)+\s//g;#with
			#$title =~ s/[\b$]//;
...
sub build_bigrams{
	...
	#while ($word ne "\n" && $word ne "" && $count < $SEQUENCE_LENGTH)
	#seq=seed_word
	# seq+=" "+word
	foreach my $title (@tracks){
		my $i=0;
		my @words = split /\s+/, $title ; #split to words
		my $len = @words;
		#print "@words\n" ;
		if($len >1){ 
			while($len>$i+1){
			$counts{@words[$i]}{@words[$i+1]}++; #adjacent
			$i++;
		}
}
}
...
sub mcw{
	...
	my $test = 0;
	  ...
	  foreach $key (sort(keys %{$counts{$word}})) {
	  		if ($test<$counts{$word}{$key} && !(exists($word_history{$key}))){
	  			$test = $counts{$word}{$key};
	  			$best_word = $key;
	  		}
